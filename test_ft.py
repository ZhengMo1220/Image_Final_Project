...
import numpy as np
import os
from main import predict_mask, dice_coef

BASE = os.path.join(os.path.dirname(__file__), "MRIsample")
FILES = sorted(
    [f for f in os.listdir(os.path.join(BASE, "FT")) if f.endswith(".jpg")],
...
def load_data():
    ...
    for fname in FILES:
        ...
        gt = cv2.imread(os.path.join(BASE, "FT", fname), cv2.IMREAD_GRAYSCALE)
...
if __name__ == "__main__":
    ...
    print(f"FT Dice mean: {mean_dice:.3f}")
